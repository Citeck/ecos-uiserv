...
internal class JournalSettingsServiceImplTest {
    ...
    fun getByIdLegacyTest() {
        ...
        setContext("user1")
        val readFully = IOUtils.readAsBytes(Thread.currentThread().contextClassLoader.getResourceAsStream(
                "test/settings/JournalSettingsServiceImplTest/old-prefs.json"))
        journalPrefService.deployOverride("old-prefs-id-test-3", readFully, "user1", JournalPrefService.TargetType.USER, "journal1")
    ...
    fun saveTest() {
        ...
        val service = JournalSettingsServiceImpl(authoritiesSupport, repo, spyPermService, journalPrefService, fileService)

        SecurityUtils.doAsUser("some-authority", object : UncheckedSupplier<Unit> {
            override fun get() {
    ...
    fun saveByAnotherUserTest() {
        ...
        Mockito.doAnswer {
            if (SecurityUtils.getCurrentUserLoginFromRequestContext() == "some-authority") {
                listOf("some-authority", "GROUP_all", "ROLE_USER")
            } else if (SecurityUtils.getCurrentUserLoginFromRequestContext() == "another-authority") {
                listOf("another-authority", "GROUP_all", "ROLE_USER")
            } else if (SecurityUtils.getCurrentUserLoginFromRequestContext() == "admin") {
                listOf("admin", "GROUP_all", "ROLE_USER", "ROLE_ADMIN")
            } else {
                fail("incorrent user - " + SecurityUtils.getCurrentUserLoginFromRequestContext())
            }
        }.`when`(authoritiesSupport).currentUserAuthorities
        val spyPermService = Mockito.spy(JournalSettingsPermissionsServiceImpl(authoritiesSupport))
        val service = JournalSettingsServiceImpl(authoritiesSupport, repo, spyPermService, journalPrefService, fileService)
        ...
        SecurityUtils.doAsUser("another-authority", object : UncheckedSupplier<Unit> {
            override fun get() {
    ...
    fun deleteLegacyRecordTest() {
        ...
        val readFully = IOUtils.readAsBytes(Thread.currentThread().contextClassLoader.getResourceAsStream(
                "test/settings/JournalSettingsServiceImplTest/old-prefs.json"))
        journalPrefService.deployOverride("old-prefs-id-test-4", readFully, "user1", JournalPrefService.TargetType.USER, "journal1")
    ...
    fun searchSettingsWithLegacyPreferences() {
        ...
        val readFully = IOUtils.readAsBytes(Thread.currentThread().contextClassLoader.getResourceAsStream(
                "test/settings/JournalSettingsServiceImplTest/old-prefs.json"))
        journalPrefService.deployOverride("old-prefs-id-test-1", readFully, "user1", JournalPrefService.TargetType.USER, "journal1")
        clearContext()
    ...
    fun searchSettingsWithLegacyPreferencesWithCollision() {
        ...
        val readFully = IOUtils.readAsBytes(Thread.currentThread().contextClassLoader.getResourceAsStream(
                "test/settings/JournalSettingsServiceImplTest/old-prefs.json"))
        journalPrefService.deployOverride("id-1-overriden", readFully, "user1", JournalPrefService.TargetType.USER, "journal1")
        clearContext()
    ...
    fun getSettingsByAuthorityAndJournalId() {
        ...
        SecurityUtils.doAsUser("auth-1", object : UncheckedSupplier<Unit> {
            override fun get() {
                service.save(JournalSettingsDto.create {
                    withId("id-1")
        ...
        SecurityUtils.doAsUser("auth-2", object : UncheckedSupplier<Unit> {
            override fun get() {
                service.save(JournalSettingsDto.create {
                    withId("id-3")
