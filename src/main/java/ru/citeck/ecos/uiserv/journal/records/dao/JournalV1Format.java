...
public class JournalV1Format implements JournalModelFormat<JournalConfigResp> {
    ...
    public JournalConfigResp convert(JournalDto dto) {
        ...
        if (dto.getGroupBy() != null) {
            meta.setGroupBy(new DataValue(dto.getGroupBy()));
        }
        if (dto.getMetaRecord() != null) {
            meta.setMetaRecord(dto.getMetaRecord().toString());
        }
        meta.setPredicate(new DataValue(dto.getPredicate()));
        if (meta.getPredicate().isNull()) {
            meta.setPredicate(new DataValue(Json.getMapper().newObjectNode()));
        }
